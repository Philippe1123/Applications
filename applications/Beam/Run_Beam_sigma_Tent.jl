...
@everywhere using Coupling_Beam_Tent
...
folder = string(pathToInterm,"/Interm10/Beam") # for report
...
nterms=100
#pt=DigitalNet64_2(nterms)
pt=LatticeRule(nterms)
@everywhere MatlabSampler() = eval_string(string("Solver_L_JULIA_MATLAB_READ_IN_sigma_Tent(", myid(), ")"))
init_Beam_mlqmc_L_Het_Single_HigherOrder=Coupling_Beam_Tent.init_Beam(SL(),true,false,false,false,true,true,nb_of_warm_up_samples=2,max_level=1,startlevel=1,nshifts=8,MatlabSampler,folder,folder_with_elements,true,nterms=nterms,NQoI=1,numberoftol=30,smoothness=4.0,ptgen=pt,corr_len=1.0)
