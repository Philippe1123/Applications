...
folder = string(pathToInterm,"/Interm8/Beam") # for report
...
@everywhere MatlabSampler() = eval_string(string("Solver_L_JULIA_MATLAB_READ_IN_SOBOL2(", myid(), ")"))
init_Beam_mlqmc_L_Het_Single_HigherOrder=Coupling_Beam.init_Beam(SL(),true,false,false,false,true,true,nb_of_warm_up_samples=2,max_level=1,startlevel=1,nshifts=8,MatlabSampler,folder,folder_with_elements,true,nterms=nterms,NQoI=1,numberoftol=200,smoothness=6.0,ptgen=pt,corr_len=2.0)
