 ...
 #using Distributions,Interpolations, GaussianRandomFields,Atom,Revise

 using Distributions,Interpolations, GaussianRandomFields,Revise
...
function Transform_3d(Points::Array{Float64,1})
Cov=0.8
...
function interp1(xpt, ypt, x)
        ...
        intf = extrapolate(interpolate((xpt,), ypt, Gridded(Interpolations.Linear())),Flat())

         y[vec(idx)] = intf(x[vec(idx)])
...
function interp1_equi(xpt, ypt, div; method="linear", extrapvalue=nothing)
    ...
    if method == "linear"
        intf = interpolate((xpt,), ypt, Gridded(Interpolations.Linear()))
        y[idx] = intf[x[idx]]
