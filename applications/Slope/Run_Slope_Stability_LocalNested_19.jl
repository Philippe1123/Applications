...
folder = string(pathToInterm,"/Interm29/Slope") # for report
...
@everywhere MatlabSampler() = eval_string(string("Slope_Slover_JULIA_MATLAB_LocalNested_19(", myid(), ")"))
...
pt=LatticeRule(nterms)
#pt=DigitalNet64(nterms)
init_Beam_MC_L_Het_Single=Coupling_Slope_2.init_Slope(SL(),false,false,false,true,true,startlevel=0,max_level=0,MatlabSampler,folder,folder_with_elements,false,numberoftol=200,nb_of_warm_up_samples=2,nshifts=8,nterms=nterms,correlateOnlyDiffs=true,corr_len=1.5,smoothness=2.0,point_generator=pt)
estimator=init_Beam_MC_L_Het_Single
history = run(estimator,5e-14)
