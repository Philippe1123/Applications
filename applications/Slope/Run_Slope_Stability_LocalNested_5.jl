...
folder = string(pathToInterm,"/Interm15/Slope") # for report
...
@everywhere MatlabSampler() = eval_string(string("Slope_Slover_JULIA_MATLAB_LocalNested_5(", myid(), ")"))
...
init_Beam_MC_L_Het_Single=Coupling_Slope_Tent.init_Slope(SL(),true,false,false,true,true,startlevel=0,max_level=0,MatlabSampler,folder,folder_with_elements,false,numberoftol=200,nb_of_warm_up_samples=2,nshifts=8,nterms=nterms,correlateOnlyDiffs=true,corr_len=1.5,smoothness=2.0,point_generator=pt)
estimator=init_Beam_MC_L_Het_Single
