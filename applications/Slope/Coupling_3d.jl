...
using GaussianRandomFields,Interact,MultilevelEstimators,Interpolations,MATLAB,Pkg,FieldTransformation
using Distributions
using Distributed
...
function init_3d(index_set::AbstractIndexSet, is_qmc::Bool, is_multiple_qoi::Bool, is_analyse::Bool,isField::Bool,isHigerOrderRefinement::Bool,MatlabSampler::Function,folder_Interm::String,folder_with_elements::String,GaussPoints::Bool; corr_len::T=1.0, smoothness::T=1.0, nterms::N=101,
     max_level::N=3, nshifts::N=1,nb_of_warm_up_samples::N=40,continuate::Bool=true,numberoftol::N=10,NQoI::N=1,correlateOnlyDiffs::Bool=false,kwargs...) where{T<:AbstractFloat,N<:Integer,V<:MSession}
...
#println("-------------------------------------------------------------------------")
    ...
    println("h-ref")
      ...
      Nodes_elements=Nodes_elements[:,2:4]
  ...
  #  end
    ...
    if(correlateOnlyDiffs==true)
     ...
     #  exp_field = GaussianRandomFields.Exponential(corr_len,σ=smoothness,p=p)
       ...
       println(p)
       cov = CovarianceFunction(3,exp_field)
       ...
       if(correlateOnlyDiffs==false)
         Random.seed!(1234)
         grf= GaussianRandomField(cov,CirculantEmbedding(),range(0, stop=1, length=201),range(0, stop=5, length=201),range(0, stop=0.5, length=201),minpadding=2)
  ...
  #              grfs[index] = GaussianRandomField(cov,KarhunenLoeve(nterms),Nodes[index],Elements[index],quad=GaussLegendre())
        ...
        #correlateOnlyDiffs==true
          ...
          for index in indices
            ...
            Random.seed!(1234)
            grfs[index][index]=GaussianRandomField(cov,KarhunenLoeve(nterms),Nodes[index][index],Elements[index],quad=GaussLegendre())
          ...
          #    println(Nodes)
              Random.seed!(1234)
              grfs[key][index] = GaussianRandomField(cov,KarhunenLoeve(nterms),Nodes[key][index],Elements[index],quad=GaussLegendre())
    #          println("here")
      ...
      increment=0
        ...
        sample_function =  (index, ξ) -> ThreeD(index, ξ, grfs,Nodes,Elements,MatlabSampler,folder_Interm,folder_with_elements,isHigerOrderRefinement,isElementRefinement,increment,correlateOnlyDiffs)
...
function ThreeD(index::Index, ξ::Vector{T} where {T<:Real}, grf::Dict, Nodes::Dict, Elements::Dict,MatlabSampler::Function,folder::String,folder_with_elements::String,isHigerOrderRefinement::Bool,isElementRefinement::Bool,increment::Int64,correlateOnlyDiffs::Bool)
...
#    Zf = GaussianRandomFields.sample(grf[index],xi=ξ) # compute GRF
    Zf = my_grf_sample(grf[index],randn(GaussianRandomFields.randdim(grf[index]))) # compute GRF
    Zf=vec(reshape(Zf,201^3,1,1))
  #  Zf=expfield(Zf)
  Zf=FieldTransformation.Transform_3d(Zf)
 Zf=vec(Zf)
    Qf = ThreeD_Sample(Zf,MatlabSampler,folder,index,false,isHigerOrderRefinement,isElementRefinement,StepChange)
    ...
    #    index_1=index[1]-1
        ...
        Zc=Zf
    #    Zc=expfield(Zc)
    #    Zc=FieldTransformation.Transform_L(Zc)
        Qc = ThreeD_Sample(Zc,MatlabSampler,folder,index_1,true,isHigerOrderRefinement,isElementRefinement,StepChange)
        dQ += value*Qc
...
## sample functions ##
function ThreeD_Sample(Z::Vector{T},MatlabSampler::Function,folder::String,index::Index,doRestriction::Bool,isHigerOrderRefinement::Bool,isElementRefinement::Bool,StepChange::Int64) where {T<:Real}

#println("Sample")
#println(string(folder,string("/Res_",myid())))
