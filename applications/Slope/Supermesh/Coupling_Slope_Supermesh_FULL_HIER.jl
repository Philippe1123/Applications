module Coupling_Slope_Supermesh_FULL_HIER
...
function init_Slope(index_set::AbstractIndexSet, is_qmc::Bool, is_multiple_qoi::Bool, is_analyse::Bool,isField::Bool,isHigerOrderRefinement::Bool,MatlabSampler::Function,folder_Interm::String,folder_with_elements::String,GaussPoints::Bool; corr_len::T=1.0, smoothness::T=1.0, nterms::N=101,
     max_level::N=3, nshifts::N=1,nb_of_warm_up_samples::N=40,continuate::Bool=true,numberoftol::N=10,NQoI::N=1,correlateOnlyDiffs::Bool=false,point_generator::Random.AbstractRNG,Supermesh::Bool=true,kwargs...) where{T<:AbstractFloat,N<:Integer,V<:MSession}
...
println(distributions[1])
  ...
  #                Random.seed!(7328)
    #              println(GaussianRandomFields.sample(grfs[index])[13])
        ...
        #  elseif(Supermesh==true)
            #    Random.seed!(1234)
            #    grfs = GaussianRandomField(cov,KarhunenLoeve(nterms),NodesSupermesh,Elements[Index(0)],quad=GaussLegendre())
...
#              println("here")
        ...
        if(Supermesh==true)
        PathNodes=string(string(folder_with_elements,string("/",type,"_refinement/GaussPoints")),"_SUPERMESH_FULL_HIER.txt")
        Handle_Nodes=open(PathNodes)
        Node=readdlm(Handle_Nodes);
        close(Handle_Nodes)
        Node=Node[:,2:3]
        NodesSupermesh[Index(0)]=Node
        Random.seed!(1234)
        grfs[Index(0)] = GaussianRandomField(cov,KarhunenLoeve(nterms),NodesSupermesh[Index(0)],Elements[Index(0)],quad=GaussLegendre())
        end
...
function Slope_Supermesh(index::Index, ξ::Vector{T} where {T<:Real}, grf::Dict,Nodes::Dict,NodesSuper::Dict,Elements::Dict,MatlabSampler::Function,folder::String,folder_with_elements::String,isHigerOrderRefinement::Bool,isElementRefinement::Bool,increment::Int64,correlateOnlyDiffs::Bool)
    grf=grf[Index(0)]
  #  NodesSuper=NodesSuper[Index(0)]
    StepChange=0
   if(length(increment)==length(index))
    ...
    Nodes_Super=NodesSuper[Index(0)]

    Nodes_Fine=Nodes[index]
    Elements_Fine=Elements[index]
    Z_Super = GaussianRandomFields.sample(grf,xi=ξ) # compute GRF
    Z_Super=expfield(Z_Super)
    itp = ScatteredInterpolation.interpolate(NearestNeighbor(), Nodes_Super', Z_Super);
    Zf=evaluate(itp, Nodes_Fine')
    Zf=vec(Zf)
...
#println("Slope_cslo")
  ...
  if(grfs[index_1][index].data.eigenfunc[1,1]==grfs[index][index].data.eigenfunc[1,1])
        Zc=GaussianRandomFields.sample(grfs[index_1][index],xi=ξ)
       Zc=expfield(Zc)
  #      println("success")
      else
  #      println("fail")
