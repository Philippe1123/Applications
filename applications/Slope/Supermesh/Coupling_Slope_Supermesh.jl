module Coupling_Slope_Supermesh
    ...
    println("h-ref")
      ...
      Nodes_elements=Nodes_elements[:,2:3]
        ...
        elseif(Supermesh==true)
          ...
          PathNodes=string(string(folder_with_elements,string("/",type,"_refinement/GaussPoints_L",Access)),"_SUPERMESH.txt")
          Handle_Nodes=open(PathNodes)
          Node=readdlm(Handle_Nodes);
          close(Handle_Nodes)
          Node=Node[:,2:3]
          NodesSupermesh[id]=Node
    ...
    #              println(GaussianRandomFields.sample(grfs[index])[13])
          ...
          elseif(Supermesh==true)
            for index in indices
                println(index)
                Random.seed!(1234)
                grfs[index] = GaussianRandomField(cov,KarhunenLoeve(nterms),NodesSupermesh[index],Elements[index],quad=GaussLegendre())
                println(grfs[index])
              end
...
function Slope_Supermesh(index::Index, ξ::Vector{T} where {T<:Real}, grf::Dict,Nodes::Dict,NodesSuper::Dict,Elements::Dict,MatlabSampler::Function,folder::String,folder_with_elements::String,isHigerOrderRefinement::Bool,isElementRefinement::Bool,increment::Int64,correlateOnlyDiffs::Bool)
   ...
   if(length(increment)==length(index))
    ...
    Nodes_Super=NodesSuper[index]

    Nodes_Fine=Nodes[index]
    Elements_Fine=Elements[index]
    Z_Super = GaussianRandomFields.sample(grf[index],xi=ξ) # compute GRF
