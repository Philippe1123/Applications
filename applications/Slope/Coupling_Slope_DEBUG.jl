  ...
  #  end
    ...
    if(correlateOnlyDiffs==true)
     ...
     #  exp_field = GaussianRandomFields.Exponential(corr_len,σ=smoothness,p=p)
       exp_field = GaussianRandomFields.Matern(corr_len,2.0,σ=smoothness,p=p)
       ...
       if(correlateOnlyDiffs==false)
            for index in indices
                ...
                println(grfs[index].data.eigenfunc[1,1])
#                Random.seed!(1234)
...
#Random.seed!(1234)
#l0=GaussianRandomFields.sample(grfs[Level(2)][Level(3)])
#Random.seed!(1234)
#l1=GaussianRandomFields.sample(grfs[Level(3)][Level(3)])
#println(l0[1])
#println(l1[1])

#println(Nodes[Level(1)][Level(2)][1][1])
#println(Nodes[Level(2)][Level(2)][1][1])

#println(size(grfs[Level(2)][Level(3)].data.eigenfunc))
#println(size(grfs[Level(3)][Level(3)].data.eigenfunc))

#println(grfs[Level(2)][Level(3)].data.eigenfunc[127,6])
#println(grfs[Level(3)][Level(3)].data.eigenfunc[176,6])


#if(grfs[Level(1)][Level(2)].data.eigenfunc[317,6]==grfs[Level(2)][Level(2)].data.eigenfunc[317,6])
#println("Succes")
#else
#  println("Fail")
#end
#Dig=LatticeRule(nterms)

folder=string("/home/philippe/Desktop/RunFiles/Trash",name)
...
#        Zc=vec(Zc)
  if(grfs[index_1].data.eigenfunc[1,1]==grfs[index].data.eigenfunc[1,1])
